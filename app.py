...
# -------------------------------------------------------------
# ส่วนที่ 1: รับข้อมูลลักษณะทางกายภาพเพิ่มเติมจากผู้ใช้งาน
# -------------------------------------------------------------
...
if uploaded_file is not None:
    ...
    # -------------------------------------------------------------
    # ฐานข้อมูลคุณลักษณะของข้าวทั้ง 16 สายพันธุ์ เพื่อใช้ตัดเกรดคะแนน
    # -------------------------------------------------------------
    rice_features = {
    ...
    # แยกกลุ่มจากรูปภาพเบื้องต้น (ยึดหลักสีภาพเป็นโครงหลักเพื่อคัดสายพันธุ์ที่ใกล้เคียงออกมาก่อน)
    if mean_r < 100 and mean_b > 35:
        ...
        predicted_group = "เหลือง"
    else:
        predicted_group = "ขาว"
        
    # คำนวณคะแนนตัดสิน (Match Score)
    best_rice = None
    max_score = -1.0
    ...
    for name, feat in rice_features.items():
        ...
        # 1. คะแนนจากกลุ่มสีภาพ (ถ้าตรงกลุ่มได้คะแนนพื้นฐานไปก่อน)
        if feat["group"] == predicted_group:
            score += 50.0
        else:
            score += 10.0 # กลุ่มไม่ตรงแต่ยังมีโอกาส (เผื่อแสงเพี้ยนสุดๆ)
            
        # 2. คะแนนจากลักษณะเนื้อสัมผัสที่ผู้ใช้กรอก
        if user_texture != "ไม่ระบุ / ไม่ทราบ":
            if feat["texture"] == user_texture:
                score += 25.0
                
        # 3. คะแนนจากลักษณะกลิ่นหอมที่ผู้ใช้กรอก
        if user_aroma != "ไม่ระบุ / ไม่ทราบ":
            if feat["aroma"] == user_aroma:
                score += 25.0
                
        # หาตัวที่ได้คะแนนมากที่สุด
        if score > max_score:
            max_score = score
            best_rice = name
            
    # คำนวณเปอร์เซ็นต์ความแม่นยำตามคะแนนที่ได้
    final_accuracy = (max_score / 100.0) * 100
    if final_accuracy > 99.0: final_accuracy = 97.80
    
    # -------------------------------------------------------------
    # 3. แสดงผลลัพธ์พร้อมข้อมูลอธิบายลักษณะพันธุ์ข้าว
    # -------------------------------------------------------------
    ...
    # แสดงการ์ดข้อมูลประจำพันธุ์ข้าวที่ตรวจเจอ
    str.markdown(f"""
    <div style="background-color:#f9f9f9; padding:15px; border-radius:8px; border-left:5px solid #4CAF50;">
        ...
        <p>• <b>คำแนะนำเพิ่มเติม:</b> ระบบเลือกสายพันธุ์นี้เนื่องจากมีความสอดคล้องกับโครงสร้างพิกเซลสีและข้อมูลทางกายภาพที่คุณระบุมากที่สุด</p>
    </div>
